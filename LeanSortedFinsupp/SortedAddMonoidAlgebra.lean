...
lemma ext_iff [DecidableEq G] {l1 l2 : SortedAddMonoidAlgebra R G cmp} :
    l1 = l2 ↔ ∀ s : G, l1 s = l2 s := SortedFinsupp.ext_iff

variable (cmp) in
def single (x : G) (y : R) [Decidable (y = 0)] : SortedAddMonoidAlgebra R G cmp :=
...
@[simp]
lemma zero_apply [DecidableEq G] (x : G) : (0 : SortedAddMonoidAlgebra R G cmp) x = 0 := rfl

@[simp]
lemma coe_zero  [DecidableEq G] : ⇑(0 : SortedAddMonoidAlgebra R G cmp) = 0 := rfl

@[simp]
lemma zero_sum [DecidableEq G] [DecidableEq R]
    {R' : Type*} [AddCommMonoid R'] (g : G → R → R') :
    (0 : SortedAddMonoidAlgebra R G cmp).sum g = 0 := rfl
...
variable (l₁ l₂ : SortedAddMonoidAlgebra R G cmp)

instance instMul : Mul (SortedAddMonoidAlgebra R G cmp) where
...
-- def
#check AddMonoidAlgebra.singleZeroAlgHom
instance : Algebra M (SortedAddMonoidAlgebra R G cmp) where
...
def algEquivAddMonoidAlgebra [DecidableEq G] :
    (SortedAddMonoidAlgebra R G cmp) ≃ₐ[M] (AddMonoidAlgebra R G) := {
    ringEquivAddMonoidAlgebra with
    commutes' r := by
      simp [ringEquivAddMonoidAlgebra]
      ext x
      simp [AddMonoidAlgebra.single_apply]
      convert_to (single cmp 0 (algebraMap M R r) : SortedAddMonoidAlgebra R G cmp) x = _
      simp
  }
...
#synth Mul (SortedAddMonoidAlgebra R G cmp)
